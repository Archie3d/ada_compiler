                                ...
                                Type* rightType)
{
    ...
    std::string order = newTemp();
    line(order + " =w call $__ada_string_compare(l " + left.name + ", w " + leftLength.name + ", l " + right.name
         + ", w " + rightLength.name + ")");
    std::string result = newTemp();
    line(result + " =w " + comparisonInstruction(op, 'w') + " " + order + ", 0");
    return Value { result, 'w' };
}

// Two records are equal when every component is, which is what Ada says and
// also the only sound reading: the padding between components is nobody's.
Value QbeEmitter::compareRecords(const Value& left, const Value& right, Type* type)
{
    ...
    for (const FieldInfo& field : record->fields) {
        Value leftSlot = left;
        Value rightSlot = right;
        if (field.offset != 0) {
            std::string movedLeft = newTemp();
            std::string movedRight = newTemp();
            line(movedLeft + " =l add " + left.name + ", " + std::to_string(field.offset));
            line(movedRight + " =l add " + right.name + ", " + std::to_string(field.offset));
            leftSlot = Value { movedLeft, 'l' };
            rightSlot = Value { movedRight, 'l' };
        }

        Type* fieldType = baseType(field.type);
        Value equal;
        if (fieldType->kind == TypeKind::Record) {
            equal = compareRecords(leftSlot, rightSlot, fieldType);
        } else if (fieldType->kind == TypeKind::Array) {
            equal = compareArrays(BinaryOp::Equal, leftSlot, field.type, rightSlot, field.type);
        } else {
            Value leftValue = loadFrom(leftSlot, field.type);
            Value rightValue = loadFrom(rightSlot, field.type);
            std::string test = newTemp();
            line(test + " =w " + comparisonInstruction(BinaryOp::Equal, leftValue.type) + " " + leftValue.name + ", "
                 + rightValue.name);
            equal = Value { test, 'w' };
        }

        std::string combined = newTemp();
        line(combined + " =w and " + same + ", " + equal.name);
        same = combined;
    }
